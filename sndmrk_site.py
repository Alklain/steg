...
def get_new_fi(Oldfi, Nmax, Step, Nfft, sr):
    new_fi = Oldfi + (math.pi * 2 * (
                Step / ((1 / ((Nmax / Nfft) * sr)) * sr) - math.floor(Step / ((1 / ((Nmax / Nfft) * sr)) * sr)))) * (
                         180 / math.pi)
    return new_fi
...
def InsertBitToSpekMat(Nt, SpMat, sr, Step, f1, f2, grf_on=1,pwr=0):
    Ntm = Nt
