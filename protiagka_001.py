import pandas as pd
import math
import cmath
import random
import numpy as np
import matplotlib as mpl
import matplotlib.pylab as plt
import seaborn as sns
import soundfile as sf
import playsound
import wavio
from glob import glob
from docx import Document
from docxtpl import DocxTemplate
import librosa.display
import librosa
import IPython.display as ipd
from playsound import playsound
from sndmrk import *
import copy
#filename='C:/py_test_temp/kladbische.mp3'
filename='C:/py_test_temp/Kassa.wav'
#filename='/content/drive/MyDrive/01-echo_a0987.wav'
from PIL import Image
# читаем файл
y, sr = librosa.load(filename,sr=None)

y1=y[190000:195000]

Nsmp=len(y1)
#fs=1000 # частота синусоиды
fs=643 # частота синусоиды
Ts=1/fs # период синусоиды
tv = np.linspace(0, Nsmp/sr, Nsmp)
y1 = np.sin(2 * math.pi * tv * fs)

Nfft = 2048
Step = 512
#CompSp, LogSp, Step = GetMatrixSpectrum(y1, Nfft, Step )
CompSp, LogSp, Step = GetMatrixSpectrumNoWin(y1, Nfft, Step )
#ys = GetSignalFromSpectrum(CompSp, Step)
ys = GetSignalFromSpectrum_hanning(CompSp, Step)
NewMarkSp=copy.deepcopy(CompSp)
NumBlk=len(CompSp[:][1])
i=0
Ham_win = np.hanning ( Nfft )
while i < NumBlk:
    sp=copy.deepcopy(CompSp[:,i])
    sp3=copy.deepcopy(sp)


    if i>=1:
        sp_n = copy.deepcopy(CompSp[:, i-1])
        sp3_n = copy.deepcopy(sp_n)


    if i==3:
        # меняем фазу гармоники
        tls = list(abs(sp_n[:round(Nfft / 2)]))
        Nmax = tls.index(max(tls))
        Mod_Fi = SetComplexRotationFi(sp_n[Nmax], 90)
        sp_n = cange_dat_in_complex_spec(sp3, Nmax, Mod_Fi)

        # меняем фазу гармоники
        #tls = list(abs(sp[:round(Nfft / 2)]))
        #Nmax = tls.index(max(tls))
        #Mod_Fi = SetComplexRotationFi(sp[Nmax], 90)
        #sp = cange_dat_in_complex_spec(sp3, Nmax, Mod_Fi)

        tls=list(abs(sp3[:round(Nfft/2)]))
        Nmax=tls.index(max(tls))
        ysw = np.fft.ifft(sp)
        wdat = ysw.real * Ham_win
        ysw_n = np.fft.ifft(sp_n)
        wdat_n = ysw_n.real * Ham_win

        ytmp=np.zeros(Nfft+Step)
        ytmp[len(ytmp)-Nfft:]=wdat
        plt.figure(i)
        plt.plot(wdat_n)
        plt.plot(ytmp)
        plt.show()
        #get_fi(sp[Nmax])
        print(get_fi(sp[Nmax]),get_fi(sp_n[Nmax]))
        # оценка набега фазы
        dop_fi=(((Nmax/Nfft)*sr)*(Step/sr)%(math.pi*2))*(180/math.pi)
        dop_fi1 = (fs * (Step / sr) % (math.pi * 2)) * (180 / math.pi)
        print(dop_fi, dop_fi1)
        print('ok')

        break




        # начинаем анализ фазы

    NewMarkSp[:, i] = sp3
    i+=1

ys1 = GetSignalFromSpectrum(NewMarkSp, Step)

## детектировование сигнала
print('start detection')
CompSpD, LogSpD, Step = GetMatrixSpectrum(ys1, Nfft, Step )
NewMarkSpD=copy.deepcopy(CompSp)
for i in range(NumBlk):
    sp = copy.deepcopy(CompSpD[:, i])
    sp3 = copy.deepcopy(sp)
    NewMarkSpD[:, i] = sp3

ys2 = GetSignalFromSpectrum(NewMarkSpD, Step)

#plt.plot(ys-ys1)
#plt.plot(ys2-ys1)
#
#plt.plot(np.array(max_freq2))
#plt.plot(np.array(max_freq)+1)
#plt.figure()
#plt.plot(ys1)
#plt.show()
